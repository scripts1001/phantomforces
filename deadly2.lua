...
local HighlightDefult = {Enabled = false, FillColor = Color3.fromRGB(255,255,255), OutlineColor = Color3.fromRGB(0,0,0), OutlineTransparency = 0.5, FillTransparency= 0.5, AllWaysShow = false,};
...
local esp = {
	settings = {
		...
		Highlight = table.clone(HighlightDefult),
...
ChamsSection:CreateToggle("Highlight", false, function(EHE) esp.settings.Highlight.Enabled = EHE; end);
ChamsSection:CreateToggle("Allways Show Highlight", false, function(EASH) esp.settings.Highlight.AllWaysShow = EASH; end);
ChamsSection:CreateColorPicker("Highlight Outline", Color3.fromRGB(255, 255, 255), function(ehf) esp.settings.Highlight.OutlineColor = ehf; end);
ChamsSection:CreateColorPicker("Highlight Fill", Color3.fromRGB(100, 0, 255), function(ehff) esp.settings.Highlight.FillColor = ehff; end);
ChamsSection:CreateSlider("Highlight Outline", 0, 50, 100, 1, function(HFF) esp.settings.Highlight.OutlineTransparency = HFF / 100; end);
ChamsSection:CreateSlider("Highlight Fill", 0, 0, 100, 1, function(HOF) esp.settings.Highlight.FillTransparency = HOF / 100; end);
	...
	if self and self.Character then
		...
		if tosro and head then
			...
			if  onScreen and IsAlive() and esp.settings.Enable then 
				...
				if esp.settings.Highlight.Enabled then
					-- Highlight
					self.Drawings.Highlight.Adornee = tosro.Parent;
					self.Drawings.Highlight.Enabled = esp.settings.Highlight.Enabled;
					self.Drawings.Highlight.Parent = Holder;


					if self.Drawings.Highlight.OutlineColor ~= esp.settings.Highlight.OutlineColor then -- + 0.0000001 FPS Or - Idk If That Even Helps
						self.Drawings.Highlight.OutlineColor = esp.settings.Highlight.OutlineColor;
					end

					if self.Drawings.Highlight.FillColor ~= esp.settings.Highlight.FillColor then
						self.Drawings.Highlight.FillColor = esp.settings.Highlight.FillColor;
					end

					if self.Drawings.Highlight.FillTransparency ~= esp.settings.Highlight.FillTransparency then
						self.Drawings.Highlight.FillTransparency = esp.settings.Highlight.FillTransparency;
					end
					if self.Drawings.Highlight.OutlineTransparency ~= esp.settings.Highlight.OutlineTransparency then
						self.Drawings.Highlight.OutlineTransparency = esp.settings.Highlight.OutlineTransparency;
					end

					if self.Drawings.Highlight.DepthMode ~= (esp.settings.Highlight.AllWaysShow and "AlwaysOnTop" or not esp.settings.Highlight.AllWaysShow and "Occluded") then
						self.Drawings.Highlight.DepthMode = (esp.settings.Highlight.AllWaysShow and "AlwaysOnTop" or not esp.settings.Highlight.AllWaysShow and "Occluded");
					end
				else
					self.Drawings.Highlight.Adornee = nil;
